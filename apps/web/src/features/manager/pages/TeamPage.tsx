...
import { managerService, type TeamUser, type ManagerProvisionRole } from "../services/manager.service";
import {
...
function useManagerTeam() {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const response = await managerService.getTeamUsers();
      setUsers((response.data?.users ?? []).filter(u => u.role === "DEVELOPER" || u.role === "CLIENT"));
    } catch (err: any) {
  ...
  const provisionUser = async (email: string, role: ManagerProvisionRole) => { await managerService.provisionUser(email, role); await fetchUsers(); };
  const suspendUser   = async (id: string) => { await managerService.suspendUser(id);  setUsers(p => p.map(u => u.id === id ? { ...u, accountStatus: "SUSPENDED" } : u)); };
  const activateUser  = async (id: string) => { await managerService.activateUser(id); setUsers(p => p.map(u => u.id === id ? { ...u, accountStatus: "ACTIVE" }    : u)); };
  const resendInvite  = async (email: string) => { await managerService.resendInvite(email); };

  return { users, isLoading, error, provisionUser, suspendUser, activateUser, resendInvite };
}

// ─── Add User Modal ───────────────────────────────────────────────────────────
function AddUserModal({ onClose, onSubmit }: {
  onClose: () => void;
  onSubmit: (email: string, role: ManagerProvisionRole) => Promise<void>;
}) {
  const [email, setEmail]           = useState("");
  const [role, setRole]             = useState<ManagerProvisionRole>("DEVELOPER");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError]           = useState<string | null>(null);

  const ROLES: { value: ManagerProvisionRole; label: string; description: string }[] = [
...
export function ManagerTeamPage() {
  const { users, isLoading, error, provisionUser, suspendUser, activateUser, resendInvite } = useManagerTeam();

  const [activeTab, setActiveTab]         = useState<"ALL" | "DEVELOPER" | "CLIENT">("ALL");
  const [showModal, setShowModal]         = useState(false);
  const [confirmAction, setConfirmAction] = useState<{ type: "suspend" | "activate"; user: TeamUser } | null>(null);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      <PageHeader
        title="Team & Clients"
        subtitle="Manage developers and clients on your projects"
        action={
      ...
      <Table
        headers={["Member", "Role", "Status", "Added", "Actions"]}
        loading={isLoading}
        error={error}
        empty={filtered.length === 0 ? (
          <div style={{ padding: "60px", textAlign: "center" }}>
            <p style={{ fontSize: "13px", fontWeight: 500, color: "var(--color-text-primary)", marginBottom: "16px" }}>No members yet</p>
            <button onClick={() => setShowModal(true)} style={{
